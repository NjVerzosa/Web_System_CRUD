...
if (isset($_POST['email']) && isset($_POST['password'])
	...
	$user_data = 'gmail='. $email. '&name='. $name;
	...
	if (empty($name)) {
		...
		if (mysqli_num_rows($result) > 0) {
           ...
           if ($result2) {
				header("Location: login.php?success=You are registered successfully&$user_data");
		        exit();
           }else {
	           	header("Location: signup.php?error=You're try to insert SQL Injection Attack&$user_data");
		        exit();
           }
		}
	}
